...
$sql_faturamento_diario = "SELECT DATE(data_venda) as dia, SUM(valor_total) as total FROM vendas WHERE usuario_id = ? AND data_venda >= CURDATE() - INTERVAL 6 DAY AND DATE(data_venda) <= CURDATE() AND status = 'finalizada' GROUP BY DATE(data_venda) ORDER BY DATE(data_venda) ASC";
...
for ($i = 6; $i >= 0; $i--) {
    ...
    $data_faturamento_map[$date] = 0; // Inicializa com 0
}
...
$sql_vendas_categoria = "SELECT c.nome, COUNT(vi.id) as total_vendas FROM venda_itens vi JOIN produtos p ON vi.produto_id = p.id JOIN categorias c ON p.categoria_id = c.id JOIN vendas v ON vi.venda_id = v.id WHERE v.usuario_id = ? AND v.status = 'finalizada' AND MONTH(v.data_venda) = MONTH(CURDATE()) AND YEAR(v.data_venda) = YEAR(CURDATE()) GROUP BY c.nome ORDER BY total_vendas DESC";
...
$sql_top_produtos = "SELECT p.nome, SUM(vi.quantidade) as total_vendido FROM venda_itens vi JOIN produtos p ON vi.produto_id = p.id JOIN vendas v ON vi.venda_id = v.id WHERE v.usuario_id = ? AND v.status = 'finalizada' AND MONTH(v.data_venda) = MONTH(CURDATE()) AND YEAR(v.data_venda) = YEAR(CURDATE()) GROUP BY p.nome ORDER BY total_vendido DESC LIMIT 5";
...
$insight_melhor_dia_stmt = $pdo->prepare("SELECT DAYNAME(data_venda) as dia_pt, SUM(valor_total) as total FROM vendas WHERE usuario_id = ? AND data_venda >= CURDATE() - INTERVAL 6 DAY AND DATE(data_venda) <= CURDATE() AND status = 'finalizada' GROUP BY DAYNAME(data_venda), DATE_FORMAT(data_venda, '%w') ORDER BY total DESC LIMIT 1");

$insight_melhor_dia_stmt->execute([$usuario_id]);
$melhor_dia = $insight_melhor_dia_stmt->fetch(PDO::FETCH_ASSOC);

$insight_produto_campeao_stmt = $pdo->prepare("SELECT p.nome FROM venda_itens vi JOIN produtos p ON vi.produto_id = p.id JOIN vendas v ON vi.venda_id = v.id WHERE v.usuario_id = ? AND v.status = 'finalizada' AND v.data_venda >= CURDATE() - INTERVAL 6 DAY AND DATE(v.data_venda) <= CURDATE() GROUP BY p.nome ORDER BY SUM(vi.quantidade) DESC LIMIT 1");
